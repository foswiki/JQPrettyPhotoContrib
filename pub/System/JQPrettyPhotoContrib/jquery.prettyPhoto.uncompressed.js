...
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = $.extend({
			...
			show_title: true, /* true/false */
                        title_attr: 'alt', /* attribute that holds the image description */
                        description_attr: 'title', /* attribute that holds the image description */
                        social: true, /* enable to inline the social_tools */
		...
		*/
		$.prettyPhoto.initialize = function() {
			...
			if(settings.ie6_fallback && $.browser.msie && parseInt($.browser.version, 10) == 6) {
		...
		*/
		$.prettyPhoto.open = function(event) {
			...
			if(settings.social){
				facebook_like_link = settings.social_tools.replace('{location_href}', encodeURIComponent(window.location.href)); 
		...
		*/
		...
		function _build_overlay(caller){
			// Inject Social Tool markup into General markup
                        var facebook_like_link;

			if(settings.social)
				facebook_like_link = settings.social_tools.replace('{location_href}', encodeURIComponent(window.location.href)); 

			settings.markup=settings.markup.replace('{pp_social}',(settings.social)?facebook_like_link:''); 
	...
	function clearHashtag(){
		...
		if(hashtag) window.location.hash = "!prettyPhoto";
	}
